...
export default function ClientDetailPage({ params }) {
  ...
  const [convertId,    setConvertId]    = useState(null); // quotation being converted
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (cRes.status === 401 || qRes.status === 401) { router.push('/login'); return; }
        if (!cRes.ok) { router.push('/clients'); return; }
        const cData = await cRes.json();
        const qData = qRes.ok ? await qRes.json() : { quotations: [] };
  ...
  async function deleteQuotation() {
    ...
    try {
      ...
      if (!res.ok) { toast((await res.json().catch(() => ({}))).error || 'Failed to delete.', 'error'); return; }
  ...
  function whatsappMessage(q, isRelease = false) {
    const ro   = q.releaseOrder;
    const type = isRelease ? 'Release Order' : 'Quotation';
    const num  = isRelease ? ro?.orderNumber  : q.quotationNumber;
    const lines = [
      `*${type} from PainterPro*`,
      `${type} #: ${num}`,
      `Client: ${client?.name || ''}`,
      `Date: ${new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}`,
      ``,
      `*Products:*`,
      ...(q.items || []).map((it, i) => `${i + 1}. ${it.productName}${it.size ? ` (${it.size.qty}${it.size.unit} × ${it.quantity})` : ''} — ₹${fmtINR(it.amount)}`),
      ``,
      ...(q.discountAmount > 0 ? [`Subtotal: ₹${fmtINR(q.totalAmount)}`, `Discount: −₹${fmtINR(q.discountAmount)}`] : []),
      `*Total: ₹${fmtINR(q.grandTotal || q.totalAmount)}*`,
  ...
  const regular  = quotations.filter((q) => !q.releaseOrder);
  const released = quotations.filter((q) => !!q.releaseOrder);
  ...
  return (
    <div className="h-full flex flex-col gap-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-2xl font-bold text-[var(--color-np-text)] truncate">{client.name}</h1>
            {client.company && <p className="text-sm text-[var(--color-np-muted)] truncate">{client.company}</p>}
          </div>
        </div>
        <button onClick={() => router.push(`/quotations/new?clientId=${id}`)}
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-[var(--color-np-red)] text-white text-sm font-semibold rounded-xl hover:bg-[var(--color-np-red-dark)] transition-colors shadow-sm self-start sm:self-auto flex-shrink-0">
      ...
      {/* Client info card */}
      <div className="bg-white rounded-2xl border border-[var(--color-np-border)] shadow-sm p-5">
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4">
          {[
            { label: 'Mobile',  val: client.mobile || '—' },
            { label: 'Email',   val: client.email  || '—' },
            { label: 'Type',    val: client.clientType ? client.clientType.charAt(0).toUpperCase() + client.clientType.slice(1) : '—' },
            { label: 'Status',  val: client.status  ? client.status.charAt(0).toUpperCase() + client.status.slice(1) : '—' },
            { label: 'Address', val: addr || '—', wide: true },
            { label: 'GST',     val: client.gstNumber || '—' },
          ].map(({ label, val, wide }) => (
            <div key={label} className={wide ? 'sm:col-span-2 md:col-span-1' : ''}>
              <p className="text-[10px] font-bold text-[var(--color-np-muted)] uppercase tracking-widest mb-0.5">{label}</p>
              <p className="text-sm font-semibold text-[var(--color-np-text)]">{val}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 border-b border-[var(--color-np-border)]">
        {[
          { key: 'quotations',    label: 'Quotations',     count: regular.length },
          { key: 'release-orders', label: 'Release Orders', count: released.length },
        ].map(({ key, label, count }) => (
          <button key={key} type="button" onClick={() => setActiveTab(key)}
            className={`px-5 py-2.5 text-sm font-bold border-b-2 transition-colors -mb-px ${activeTab === key ? 'border-[var(--color-np-red)] text-[var(--color-np-red)]' : 'border-transparent text-[var(--color-np-muted)] hover:text-[var(--color-np-text)]'}`}>
            {label}
            <span className={`ml-2 px-1.5 py-0.5 rounded-full text-[10px] font-bold ${activeTab === key ? 'bg-[var(--color-np-red)] text-white' : 'bg-[var(--color-np-border)] text-[var(--color-np-muted)]'}`}>
              {count}
            </span>
          </button>
        ))}
      ...
      {activeTab === 'quotations' && (
        regular.length === 0 ? (
          <div className="flex-1 bg-white rounded-2xl border border-[var(--color-np-border)] flex flex-col items-center justify-center gap-3 py-20">
            ...
            <p className="text-sm text-[var(--color-np-muted)]">Create a quotation to get started.</p>
            <button onClick={() => router.push(`/quotations/new?clientId=${id}`)}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--color-np-border)] bg-[var(--color-np-gray)]">
                    {['Quotation #', 'Created', 'Items', 'Total', 'Status', ''].map((h) => (
                ...
                <tbody className="divide-y divide-[var(--color-np-border)]">
                  {regular.map((q) => (
                    <tr key={q._id} className="hover:bg-[var(--color-np-cream)] transition-colors group">
                      ...
                      <td className="px-4 py-3.5 text-[var(--color-np-muted)]">{q.items?.length || 0} item{q.items?.length !== 1 ? 's' : ''}</td>
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          {client?.mobile && (
                            <button onClick={() => whatsappMessage(q)}
                          ...
                          <button onClick={() => window.open(`/quotations/${q._id}/print`, '_blank')}
                            title="Print"
                            className="p-1.5 rounded-lg text-[var(--color-np-muted)] hover:bg-[var(--color-np-gray)] transition-colors">
                          ...
                          <button onClick={() => setDeleteId(q._id)}
                            className="p-1.5 rounded-lg text-[var(--color-np-muted)] hover:text-red-600 hover:bg-red-50 transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div key={q._id} className="p-4">
                  ...
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <div className="flex flex-wrap gap-1.5 justify-end">
                      <button onClick={() => router.push(`/quotations/${q._id}`)}
                        className="px-3 py-1.5 text-xs font-semibold text-[var(--color-np-red)] border border-[var(--color-np-red)] rounded-lg">
                        Open
                      </button>
                      <button onClick={() => setConvertId(q._id)}
                        className="px-3 py-1.5 text-xs font-semibold text-blue-600 border border-blue-300 rounded-lg">
                        → RO
                      </button>
                      <button onClick={() => setDeleteId(q._id)}
                        className="px-3 py-1.5 text-xs font-semibold text-gray-500 border border-gray-300 rounded-lg hover:bg-red-50 hover:text-red-600">
                        Delete
                      </button>
                    </div>
                  </div>
      ...
      {activeTab === 'release-orders' && (
        released.length === 0 ? (
          <div className="flex-1 bg-white rounded-2xl border border-[var(--color-np-border)] flex flex-col items-center justify-center gap-3 py-20">
            ...
            <p className="text-[var(--color-np-text)] font-semibold">No release orders yet</p>
            <p className="text-sm text-[var(--color-np-muted)]">Convert a quotation to a release order from the Quotations tab.</p>
          </div>
        ) : (
          <div className="bg-white rounded-2xl border border-[var(--color-np-border)] shadow-sm overflow-hidden">
            <div className="hidden md:block">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--color-np-border)] bg-[var(--color-np-gray)]">
                    {['RO #', 'Quotation Ref', 'Released', 'Items', 'Total', ''].map((h) => (
                      <th key={h} className="text-left px-4 py-3 text-[11px] font-bold text-[var(--color-np-muted)] uppercase tracking-wide">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--color-np-border)]">
                  {released.map((q) => (
                    <tr key={q._id} className="hover:bg-[var(--color-np-cream)] transition-colors group">
                      <td className="px-4 py-3.5">
                        <span className="font-bold text-blue-700">{q.releaseOrder.orderNumber}</span>
                      </td>
                      <td className="px-4 py-3.5 text-[var(--color-np-muted)]">{q.quotationNumber}</td>
                      <td className="px-4 py-3.5 text-[var(--color-np-muted)]">{fmtDate(q.releaseOrder.releasedAt)}</td>
                      <td className="px-4 py-3.5 text-[var(--color-np-muted)]">{q.items?.length || 0} item{q.items?.length !== 1 ? 's' : ''}</td>
                      <td className="px-4 py-3.5 font-bold text-[var(--color-np-text)]">₹{fmtINR(q.grandTotal || q.totalAmount)}</td>
                      <td className="px-4 py-3.5">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          {client?.mobile && (
                            <button onClick={() => whatsappMessage(q, true)}
                              title="Send via WhatsApp"
                              className="p-1.5 rounded-lg text-green-600 hover:bg-green-50 transition-colors">
                              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                          ...
                          <button onClick={() => window.open(`/quotations/${q._id}/print?type=release`, '_blank')}
                            title="Print Release Order"
                            className="p-1.5 rounded-lg text-[var(--color-np-muted)] hover:bg-[var(--color-np-gray)] transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                            </svg>
            ...
            <div className="md:hidden divide-y divide-[var(--color-np-border)]">
              {released.map((q) => (
                <div key={q._id} className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <p className="font-bold text-blue-700">{q.releaseOrder.orderNumber}</p>
                      <p className="text-xs text-[var(--color-np-muted)] mt-0.5">Ref: {q.quotationNumber}</p>
                    </div>
                    <ROBadge />
                  </div>
                  <div className="flex items-center justify-between mt-3">
                    <div>
                      <p className="text-xs text-[var(--color-np-muted)]">{fmtDate(q.releaseOrder.releasedAt)}</p>
                      <p className="font-bold text-[var(--color-np-text)]">₹{fmtINR(q.grandTotal || q.totalAmount)}</p>
                    </div>
      ...
      {convertId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-[var(--color-np-muted)] mb-6">This quotation will be converted to a release order. A unique RO number will be assigned.</p>
