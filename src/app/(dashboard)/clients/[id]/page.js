...
export default function ClientDetailPage({ params }) {
  ...
  const [printItem,    setPrintItem]    = useState(null); // { quotation, isRelease }
  const [whatsappItem, setWhatsappItem] = useState(null); // { quotation, isRelease }
                    ...
                    <tr key={q._id} className="hover:bg-[var(--color-np-cream)] transition-colors">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center justify-end gap-1">
                          {client?.mobile && (
                            <button onClick={() => setWhatsappItem({ quotation: q, isRelease: false })}
                              title="Send via WhatsApp"
                              className="p-1.5 rounded-lg text-green-600 hover:bg-green-50 transition-colors">
                              {WA_ICON}
                            </button>
                          )}
                          <button onClick={() => setPrintItem({ quotation: q, isRelease: false })}
                            title="Print"
                  ...
                  {released.map((q) => (
                    <tr key={q._id} className="hover:bg-[var(--color-np-cream)] transition-colors">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center justify-end gap-1">
                          {client?.mobile && (
                            <button onClick={() => setWhatsappItem({ quotation: q, isRelease: true })}
                              title="Send via WhatsApp"
                              className="p-1.5 rounded-lg text-green-600 hover:bg-green-50 transition-colors">
                              {WA_ICON}
                            </button>
                          )}
                          <button onClick={() => setPrintItem({ quotation: q, isRelease: true })}
              ...
              {released.map((q) => (
                <div key={q._id} className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">Released</span>
                  </div>
                  <div className="flex items-center justify-between mt-3">
                    <div>
